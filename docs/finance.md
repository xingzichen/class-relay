...
版本 0.2 · 2026-09-08。本模块属于小程序主方案的必需功能；当前仅为文档，未发生实际收费或交易。
...
- 付款给清洁人员或布置供应方在小程序外完成；第一版只登记和核对，不触发真实付款。
...
每次复核入账或分摊调整生成不可变的公示快照，保存统计口径和截止时点。审核发布后的快照才对班内可见；公示版本不能混用旧余额与新缴费列表。支持提出账目疑问并由管理者答复，不允许家长直接改账。
...
AI 可解释已发布费用用途、分摊规则和当前公示账目，引用公示版本。余额和金额汇总由数据库确定性计算后返回，模型不能自行编造合计或把旧公示当最新。

班内有权家长可通过 AI 查询同班已公开的孩子缴费状态及金额；此为明确公示范围，不能推广为读取该孩子其他私有任务。未公开的原始凭证、私人收款账户和待核对材料不进入家长问答。AI 不入账、不复核、不发起付款或退款。
...
| finance_questions | publication_id, user_id, question, response, status；不直接修改账本 |
